...
IMG ?= akamai/akamai-operator:latest
...
# These images MUST exist in a registry and be pullable.
...
.PHONY: create-secret
create-secret: ## Create Akamai credentials secret in Kubernetes
	@echo "Creating Akamai credentials secret..."
	@kubectl create secret generic akamai-credentials \
		--from-literal=host="$(AKAMAI_HOST)" \
		--from-literal=client_token="$(AKAMAI_CLIENT_TOKEN)" \
		--from-literal=client_secret="$(AKAMAI_CLIENT_SECRET)" \
		--from-literal=access_token="$(AKAMAI_ACCESS_TOKEN)" \
